...
public class ElementSearch : AIStateInterface {
	...
	private ElementType elementToSearch;
	private float waitingSinceSeconds;

	public ElementSearch(Enemy enemy, ElementType elementToSearch) {
		// Set enemy instance
		this.enemy = enemy;
		...
		// Init search
		FindClosestElementPath();
	}

	public void FindClosestElementPath() {
		path = star.AStarPath(enemy.getCurTile(), LevelManager.instance.GetClosestTileOfType(elementToSearch, enemy.getCurTile().transform.position));
		path.Reverse();
		currentNode = 0;
	}
	...
	public void Update () {
		...
		if (star.found) {
			...
			if (!path [path.Count - 1].tile.HasElementOfType (elementToSearch) || enemy.getCurTile () == path [path.Count - 2].tile) {

				// If reached target with the element still present, set foundWater flag, remove element and return to initial point
				if (path [path.Count - 1].tile.HasElementOfType (elementToSearch)) {
					enemy.foundElement = true;
					path [path.Count - 1].tile.LoseElement ();
					currentNode = 1;

					this.enemy.setState(AIStates.ReturnSpawnElement);
